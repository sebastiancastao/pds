-- ========================================
-- FIXED SQL TO RUN IN SUPABASE SQL EDITOR
-- This version drops and recreates the table cleanly
-- ========================================

-- Step 1: Drop existing policies if they exist
DROP POLICY IF EXISTS "Admins can view all vendor invitations" ON public.vendor_invitations;
DROP POLICY IF EXISTS "Vendors can view their own invitations" ON public.vendor_invitations;
DROP POLICY IF EXISTS "Admins can create vendor invitations" ON public.vendor_invitations;
DROP POLICY IF EXISTS "Vendors can update their own invitations" ON public.vendor_invitations;
DROP POLICY IF EXISTS "Admins can update all vendor invitations" ON public.vendor_invitations;

-- Step 2: Drop the table if it exists (this will cascade and remove triggers)
DROP TABLE IF EXISTS public.vendor_invitations CASCADE;

-- Step 3: Drop the trigger function if it exists
DROP FUNCTION IF EXISTS update_vendor_invitations_updated_at() CASCADE;

-- Step 4: Create the vendor_invitations table fresh
CREATE TABLE public.vendor_invitations (
  ...
  CONSTRAINT vendor_invitations_status_check CHECK (status IN ('pending', 'accepted', 'declined', 'expired'))
);

-- Step 5: Create indexes for faster lookups
CREATE INDEX idx_vendor_invitations_token ON public.vendor_invitations(token);
CREATE INDEX idx_vendor_invitations_event_id ON public.vendor_invitations(event_id);
CREATE INDEX idx_vendor_invitations_vendor_id ON public.vendor_invitations(vendor_id);
CREATE INDEX idx_vendor_invitations_status ON public.vendor_invitations(status);

-- Step 6: Add updated_at trigger function
CREATE FUNCTION update_vendor_invitations_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
-- Step 11: Update venue_reference table with all venues and coordinates
...
-- Step 12: Create additional index for venue coordinates
