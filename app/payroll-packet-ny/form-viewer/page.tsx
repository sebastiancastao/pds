...
const NY_FORMS: FormSpec[] = [
  ...
  { id: 'temp-employment-agreement', display: 'Temporary Employment Services Agreement', requiresSignature: true, apiOverride: '/api/payroll-packet-ny/temp-employment-agreement' },
...
export default function PayrollPacketNYFormViewer() {
  return (
    <Suspense fallback={<div className="p-6 text-center">Loading form...</div>}>
      <PayrollPacketNYFormViewerContent />
    </Suspense>
  );
}

function PayrollPacketNYFormViewerContent() {
  const searchParams = useSearchParams();
  const selectedForm = searchParams.get('form');

  if (selectedForm === 'meal-waiver-10-12') {
    return <MealWaiver10to12NYPage />;
  }
  if (selectedForm === 'meal-waiver-6hour') {
    return <MealWaiver6HourNYPage />;
  }
  if (selectedForm === 'employee-information') {
    return <EmployeeInformationNYForm />;
  }

  return (
    <StatePayrollFormViewerWithSuspense stateCode="ny" stateName="New York" forms={NY_FORMS} />
  );
}
...
const EMPLOYEE_INFO_STORAGE_KEY = 'ny-employee-information';
