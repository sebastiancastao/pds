...
export default function EventDashboardPage() {
  ...
  const [locationRunners, setLocationRunners] = useState<Record<string, string>>({});
  const [settingRunnerId, setSettingRunnerId] = useState<string | null>(null);
  ...
  const availableAddVendorStates = useMemo(() => {
    const states = addVendorOptions.map((v) => v.profiles?.state).filter((s): s is string => Boolean(s));
    return [...new Set(states)].sort();
  }, [addVendorOptions]);

  const availableAddVendorCities = useMemo(() => {
    const cities = addVendorOptions
      .filter((v) => addVendorState === "all" || v.profiles?.state === addVendorState)
      .map((v) => v.profiles?.city)
      .filter((c): c is string => Boolean(c));
    return [...new Set(cities)].sort();
  }, [addVendorOptions, addVendorState]);
  ...
  const availableLocationTeamCities = useMemo(() => {
    const cities = locationTeamVendors
      .filter((v) => locationTeamState === "all" || v.profiles?.state === locationTeamState)
      .map((v) => v.profiles?.city)
      .filter((c): c is string => Boolean(c));
    return [...new Set(cities)].sort();
  }, [locationTeamVendors, locationTeamState]);

  const filteredLocationTeamVendors = useMemo(() => {
    const query = locationTeamSearchQuery.trim().toLowerCase();
    const sorted = [...locationTeamVendors]
      .filter((vendor) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({
          ...
          event_type: eventData.event_type === "special" ? "special" : "normal",
        });
  ...
  const loadLocations = async () => {
    ...
    try {
      ...
      for (const assignment of assignments) {
        ...
        if (assignment.is_runner) {
          runnerMap[assignment.location_id] = assignment.vendor_id;
        }
      }
  ...
  const handleSetLocationRunner = async (locationId: string, vendorId: string, unset = false) => {
    if (!eventId || !locationId || !vendorId) return;
    setSettingRunnerId(vendorId);
    try {
      const token = await getSessionToken();
      const res = await fetch(`/api/events/${eventId}/locations`, {
        method: "PUT",
        ...
        body: JSON.stringify({ locationId, runnerId: unset ? null : vendorId }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        alert(data?.error || "Failed to update runner");
        return;
      }
      setLocationRunners((prev) => {
        const next = { ...prev };
        if (unset) {
          delete next[locationId];
        } else {
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setForm((prev) => ({ ...prev, event_type: "normal" }))}
                    className={`flex-1 py-3 px-4 rounded-lg border-2 font-semibold text-sm transition-all duration-200 ${
                      form.event_type !== "special"
                  ...
                  <button
                    type="button"
                    onClick={() => setForm((prev) => ({ ...prev, event_type: "special" }))}
                    className={`flex-1 py-3 px-4 rounded-lg border-2 font-semibold text-sm transition-all duration-200 ${
                      form.event_type === "special"
                            ...
                            {assignedMembers.length === 0 && !(locationProposals[loc.id]?.some(p => p.status === "pending")) ? (
                              ...
                              <div className="border rounded-lg overflow-hidden">
                                {assignedMembers.map((member) => {
                                  ...
                                  const isRunner = locationRunners[loc.id] === memberId;
                                    ...
                                    >
                                      <div className="min-w-0">
                                        <div className="flex items-center gap-2 flex-wrap">
                                          ...
                                          {isLeader && (
                                            <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-yellow-100 text-yellow-800 border border-yellow-300 flex items-center gap-0.5">
                                          ...
                                          {isRunner && (
                                            <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-sky-100 text-sky-800 border border-sky-300 flex items-center gap-0.5">
                                              <svg className="w-2.5 h-2.5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                                              </svg>
                                              <svg className="w-2.5 h-2.5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        {isConfirmed && canManageLocations && (
                                          <button
                                            type="button"
                                            title={isRunner ? "Remove runner" : "Set as runner"}
                                            onClick={() => { void handleSetLocationRunner(loc.id, memberId, isRunner); }}
                                            disabled={settingRunnerId !== null}
                                            className={`p-1.5 rounded transition flex items-center ${isRunner ? "text-sky-500 hover:text-gray-400 hover:bg-gray-50" : "text-gray-400 hover:text-sky-500 hover:bg-sky-50"}`}
                                          >
                                            <svg className="w-4 h-4 -mr-1.5" viewBox="0 0 24 24" fill={isRunner ? "currentColor" : "none"} stroke="currentColor" strokeWidth={isRunner ? 0 : 1.5}>
                                              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                                            </svg>
                                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill={isRunner ? "currentColor" : "none"} stroke="currentColor" strokeWidth={isRunner ? 0 : 1.5}>
                                              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                                            </svg>
                                          </button>
                                        )}
              ...
              {(availableLocationTeamStates.length > 0 || availableLocationTeamCities.length > 0 || regions.length > 0) && (
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <select
                      value={locationTeamState}
                      onChange={(e) => { setLocationTeamState(e.target.value); setLocationTeamCity("all"); }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                      disabled={loadingLocationTeamVendors}
                    >
                      <option value="all">All States</option>
                      {availableLocationTeamStates.map((s) => (
                        <option key={s} value={s}>{s}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Filter by City</label>
                    <select
                      value={locationTeamCity}
                      onChange={(e) => setLocationTeamCity(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                      disabled={loadingLocationTeamVendors}
                    >
                      <option value="all">All Cities</option>
              ...
              {(availableAddVendorStates.length > 0 || availableAddVendorCities.length > 0 || regions.length > 0) && (
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Filter by State</label>
                    <select
                      value={addVendorState}
                      onChange={(e) => { setAddVendorState(e.target.value); setAddVendorCity("all"); }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                      disabled={loadingAddVendors || addingVendorToTeam}
                    >
                      <option value="all">All States</option>
                      {availableAddVendorStates.map((s) => (
                        <option key={s} value={s}>{s}</option>
                      ))}
                    </select>
                  </div>
