...
export async function POST(req: NextRequest) {
  try {
    ...
    const getPayrollInputsForEvent = (event: any) => {
      ...
      const tipsEligibleMembers = workers.flatMap((w: any) => {
        ...
        if (
          !workerId ||
          isTrailersDivision(w?.division) ||
          w?.payment_data?.tips_deleted === true ||
          workerHours <= 0
        ) {
          return [];
        }
    ...
    const addCommissionReportPage = (
      ...
      const { rawPayPeriodRateInEffect, payPeriodRateInEffect } = (() => {
        ...
        const raw = totals.hoursWorked > 0 ? totals.commission / totals.hoursWorked : 0;
        return { rawPayPeriodRateInEffect: raw, payPeriodRateInEffect: roundPayrollAmount(raw) };
      })();
      ...
      const totalVariableRate =
        grandHoursWorked > 0
          ? roundPayrollAmount(grandRawVariableIncentive / grandHoursWorked)
          : 0;
      const totalVariableIncentive = roundPayrollAmount(grandRawVariableIncentive);
      const totalFinalPay = roundPayrollAmount(
        roundPayrollAmount(grandCommission) +
    ...
    if (paystubState === "CA") {
      ...
      for (const event of events || []) {
        ...
        if (!isEventSD && ((adjustedGrossForReport > 0 && commissionEligibleCount > 0) || commissionReportFinalPay > 0)) {
      ...
      const normalizedCaCommissionRows = (() => {
        ...
        const rawPayPeriodRateInEffect =
      ...
      // Rest breaks shown on the paystub: this period's rest break pay divided by $4.50,
      // rounded down to a whole break (e.g. $25.00 / $4.50 = 5.55 -> 5).
      const totalRestBreakCount = Math.floor(totalRestBreakRounded / REST_BREAK_RATE + 1e-9);
      ...
      const earningsRowStartY = 200;
      const earningsRowGap = 8;
      const earningsRows = [
        { label: "Regular", color: black, rate: regularRateForEarnings, hours: regularHoursForEarnings, thisPeriod: regularPayForEarnings, ytd: ytdRegularPay, hideThisPeriod: !showHourlyEarningsRows },
        { label: "Overtime", color: black, rate: overtimeRateAvg, hours: overtimeHoursForEarnings, thisPeriod: overtimePayForEarnings, ytd: ytdOvertimePay },
        { label: "Double Time", color: black, rate: doubletimeRateAvg, hours: doubletimeHoursForEarnings, thisPeriod: doubletimePayForEarnings, ytd: ytdDoubletimePay },
        ...
        // Hours column holds this period's rest break count (a count, not hours): rest break pay / $4.50, rounded down.
      ...
      const grossPayY = earningsBottomLineY + 7;
      const statutoryHeaderY = grossPayY + 11;
      const statutoryDividerY = statutoryHeaderY + 7;
      const deductionRowStartY = statutoryDividerY + 13.1;
      const deductionRowGap = 7.4;
      ...
      const statutoryDeductionRows = [
        ...
        { label: "California State Income", thisPeriod: stateIncomeAmt, ytd: ytdStateIncome },
    ...
    if (useVendorLayout) {
      ...
      drawText("Bonus", colX.other, yPosition, { size: 8, bold: true });
    ...
    events.forEach((event: any, index: number) => {
      ...
      if (shouldRenderRow) {
        ...
        if (!isSanDiegoRegion(event) && ((adjustedGrossForReport > 0 && commissionEligibleCount > 0) || commissionReportFinalPay > 0)) {
          // Match HR dashboard payroll logic:
          // Commission Pay = distributed share, Variable Incentive = excess above that share.
