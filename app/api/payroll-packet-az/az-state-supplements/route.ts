...
export async function GET(request: Request) {
  try {
    ...
    console.log('[AZ State Supplements] Cookie-based auth:', {
      hasUser: !!user,
      userId: user?.id
    });
    ...
    if (!user) {
      ...
      if (token) {
        console.log('[AZ State Supplements] Validating Bearer token...');
        ...
        user = tokenUser?.user;
        console.log('[AZ State Supplements] Bearer token validation:', {
          hasUser: !!user,
          userId: user?.id
        });
      }
    ...
    const pdfPath = join(process.cwd(), 'az-state-supplements.pdf');
    const existingPdfBytes = readFileSync(pdfPath);
    const pdfDoc = await PDFDocument.load(existingPdfBytes);
    const pages = pdfDoc.getPages();
    const lastPage = pages[pages.length - 1];
    const lastPageSize = lastPage.getSize();
    console.log('AZ State Supplements - Page size:', lastPageSize);
    console.log('AZ State Supplements - Total pages:', pages.length);
    ...
    const backLinkAnnot = pdfDoc.context.obj({
      ...
      A: pdfDoc.context.obj({
        S: PDFName.of('URI'),
        URI: PDFString.of('/payroll-packet-az/form-viewer?form=employee-handbook')
      })
    });
    ...
    const continueLinkAnnot = pdfDoc.context.obj({
      ...
      A: pdfDoc.context.obj({
        S: PDFName.of('URI'),
        URI: PDFString.of('/payroll-packet-az/form-viewer?form=health-insurance')
      })
    });
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      ...
      headers: {
        ...
        'Content-Disposition': 'inline; filename="AZ_State_Supplements.pdf"',
        'Content-Security-Policy': "default-src 'self'",
        'X-Content-Type-Options': 'nosniff'
      }
    });
  } catch (error: any) {
    console.error('AZ State Supplements PDF error:', error);
    return NextResponse.json({
      error: 'Failed to generate AZ State Supplements PDF',
