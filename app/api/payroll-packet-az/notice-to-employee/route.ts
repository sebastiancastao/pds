  ...
  try {
    ...
    const removeFieldFromPdf = (fieldName: string) => {
      try {
        ...
        console.warn(`[NOTICE_TO_EMPLOYEE_AZ] Field not found or removal failed: ${fieldName}`, error);
      }
    };
    ...
    try {
      ...
      console.warn('[NOTICE_TO_EMPLOYEE_AZ] Failed to set employer representative name', error);
    }
    ...
    try {
      ...
      if (widgets.length > 0) {
        ...
        console.warn('[NOTICE_TO_EMPLOYEE_AZ] Employer signature field has no widgets');
      }
    } catch (error) {
    ...
    console.error('Notice to Employee PDF error (AZ):', error);
