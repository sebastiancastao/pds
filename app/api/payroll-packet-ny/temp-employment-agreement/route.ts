...
export async function GET(request: Request) {
  try {
    ...
    console.log('[NY] Cookie-based auth:', {
      hasUser: !!user,
      userId: user?.id
    });
    ...
    if (!user) {
      ...
      if (token) {
        console.log('[NY] Validating Bearer token...');
        ...
        user = tokenUser?.user;
        console.log('[NY] Bearer token validation:', {
          hasUser: !!user,
          userId: user?.id
        });
      }
    ...
    if (user) {
      ...
      console.log('[NY] Profile query result:', { profile, error });
    ...
    console.log('[NY] Final user name:', userName);
    ...
    console.log('NY - Page size:', lastPageSize);
    console.log('NY - Total pages:', pages.length);
    ...
    if (userName) {
      ...
      console.log('[NY] Drawing user name on first page at position (100, 570) size 9');
    }

    // BACK BUTTON
    const backButtonX = 50, backButtonY = 100, backButtonWidth = 100, backButtonHeight = 32;
    const backLinkAnnot = pdfDoc.context.obj({ Type: PDFName.of('Annot'), Subtype: PDFName.of('Link'), Rect: [backButtonX, backButtonY, backButtonX + backButtonWidth, backButtonY + backButtonHeight], Border: [0, 0, 0], C: [0.5, 0.5, 0.5], A: pdfDoc.context.obj({ S: PDFName.of('URI'), URI: PDFString.of('/payroll-packet-ny/form-viewer?form=notice-to-employee') }) });
    ...
    const continueLinkAnnot = pdfDoc.context.obj({ Type: PDFName.of('Annot'), Subtype: PDFName.of('Link'), Rect: [continueButtonX, continueButtonY, continueButtonX + continueButtonWidth, continueButtonY + continueButtonHeight], Border: [0, 0, 0], C: [0.1, 0.46, 0.82], A: pdfDoc.context.obj({ S: PDFName.of('URI'), URI: PDFString.of('/payroll-packet-ny/form-viewer?form=meal-waiver-6hour') }) });
