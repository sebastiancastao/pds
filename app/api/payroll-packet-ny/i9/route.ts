  ...
  try {
    ...
    const removeFieldFromPdf = (fieldName: string) => {
      try {
        ...
        console.warn(`[I9-NY] Field not found or removal failed: ${fieldName}`, error);
      }
    };
    ...
    try {
      ...
      console.warn('[I9-NY] Failed to set employer business address', error);
    }
    ...
    try {
      ...
      console.warn('[I9-NY] Failed to set employer business name', error);
    }
    ...
    try {
      ...
      if (widgets.length > 0) {
        ...
        console.warn('[I9-NY] Employer signature field has no widgets');
      }
    } catch (error) {
      console.warn('[I9-NY] Failed to embed employer signature image', error);
    }
    ...
    console.error('I-9-NY PDF generation error:', error);
