...
const ALLOWED_ROLES = ["admin", "exec", "manager", "supervisor3"];
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const role = userData.role as string;
    if (!['admin', 'exec'].includes(role)) {
      return NextResponse.json({ error: "Only admin and exec can delete planned events" }, { status: 403 });
    }
