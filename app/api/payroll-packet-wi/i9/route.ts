  ...
  try {
    ...
    const removeFieldFromPdf = (fieldName: string) => {
      try {
        ...
        console.warn(`[I9 WI] Field not found or removal failed: ${fieldName}`, error);
      }
    };
    ...
    try {
      ...
      console.warn('[I9 WI] Failed to set employer representative name/title', error);
    }
    ...
    try {
      ...
      console.warn('[I9 WI] Failed to set employer business address', error);
    }
    ...
    try {
      ...
      console.warn('[I9 WI] Failed to set employer business name', error);
    }
    ...
    try {
      ...
      if (widgets.length > 0) {
        ...
        console.warn('[I9 WI] Employer signature field has no widgets');
      }
    } catch (error) {
      console.warn('[I9 WI] Failed to embed employer signature image', error);
    }
    ...
    console.error('[I9 WI] PDF generation error:', error);
