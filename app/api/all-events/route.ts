...
export async function GET(req: NextRequest) {
  try {
    ...
    if (userRole !== 'admin' && userRole !== 'exec' && userRole !== 'hr' && userRole !== 'manager' && userRole !== 'supervisor3') {
      console.error('[ALL-EVENTS] Access denied - user role:', userRole);
      return NextResponse.json({ error: 'Access denied. Only admin, exec, hr, manager, and supervisor3 users can view all events.' }, { status: 403 });
    }
