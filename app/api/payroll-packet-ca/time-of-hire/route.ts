...
export async function GET() {
  try {
    const pdfPath = join(process.cwd(), '16_TimeOfHireNotice.pdf');
    ...
    return new NextResponse(Buffer.from(pdfBytes), { status: 200, headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'inline; filename="Time_Of_Hire_Notice.pdf"', 'Content-Security-Policy': "default-src 'self'", 'X-Content-Type-Options': 'nosniff' } });
  } catch (error: any) {
    console.error('Time Of Hire PDF error:', error);
    return NextResponse.json({ error: 'Failed to generate Time Of Hire PDF', details: error.message }, { status: 500 });
  }
}
