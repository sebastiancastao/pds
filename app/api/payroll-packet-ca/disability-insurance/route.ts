...
export async function GET() {
  try {
    ...
    const backLinkAnnot = pdfDoc.context.obj({
      ...
      Border: [0, 0, 0],
      C: [0.5, 0.5, 0.5],
      A: pdfDoc.context.obj({ S: PDFName.of('URI'), URI: PDFString.of('http://localhost:3000/payroll-packet-ca/ui-guide') })
    });
    ...
    // Create continue link annotation
    const continueLinkAnnot = pdfDoc.context.obj({
      ...
      Border: [0, 0, 0],
      C: [0.25, 0.53, 0.96],
      A: pdfDoc.context.obj({ S: PDFName.of('URI'), URI: PDFString.of('http://localhost:3000/payroll-packet-ca/paid-family-leave') })
    });
