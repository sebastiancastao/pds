...
export async function GET() {
  try {
    ...
    return new NextResponse(Buffer.from(pdfBytes), { status: 200, headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'inline; filename="Military_Veterans_Civil_Rights.pdf"', 'Content-Security-Policy': "default-src 'self'", 'X-Content-Type-Options': 'nosniff' } });
  } catch (error: any) {
    console.error('Military Rights PDF error:', error);
    return NextResponse.json({ error: 'Failed to generate Military Rights PDF', details: error.message }, { status: 500 });
  }
}
