...
export async function GET() {
  try {
    ...
    if (annotsArray instanceof PDFArray) {
      ...
      lastPage.node.set(PDFName.of('Annots'), pdfDoc.context.obj([pdfDoc.context.register(backLinkAnnot), pdfDoc.context.register(continueLinkAnnot)]));
    }
    ...
    return NextResponse.json({ error: 'Failed to generate Notice to Employee PDF', details: error.message }, { status: 500 });
  }
}
