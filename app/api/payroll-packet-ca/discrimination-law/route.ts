...
export async function GET() {
  try {
    ...
    return new NextResponse(Buffer.from(pdfBytes), { status: 200, headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'inline; filename="Discrimination_is_Against_the_Law.pdf"', 'Content-Security-Policy': "default-src 'self'", 'X-Content-Type-Options': 'nosniff' } });
  } catch (error: any) {
    console.error('Discrimination Law PDF error:', error);
    return NextResponse.json({ error: 'Failed to generate Discrimination Law PDF', details: error.message }, { status: 500 });
  }
}
