...
export async function GET(req: NextRequest) {
  try {
    ...
    const recordUserIds = [
      ...new Set((sickLeaves || []).map((row: any) => String(row.user_id || "")).filter(Boolean)),
    ];
    ...
    for (const [key, entries] of entriesByUserEvent.entries()) {
      ...
      for (const row of entries) {
        ...
        if (row.action === "clock_out" && clockIn) {
          totalForUserEvent += hoursBetween(clockIn, row.timestamp);
          clockIn = null;
        }
      }
    ...
    const accruals = activeUserIds
      .map((id) => {
        ...
        const accrued_hours = Number(
          (worked_hours / SICK_LEAVE_ACCRUAL_HOURS_WORKED).toFixed(2)
        );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const operation = operationRaw === "remove" ? "remove" : "add";
