  ...
  try {
    ...
    const removeFieldFromPdf = (fieldName: string) => {
      try {
        ...
        console.warn(`[I9 NV] Field not found or removal failed: ${fieldName}`, error);
      }
    };
    ...
    try {
      ...
      console.warn('[I9 NV] Failed to set employer representative name/title', error);
    }
    ...
    try {
      ...
      console.warn('[I9 NV] Failed to set employer business address', error);
    }
    ...
    try {
      ...
      console.warn('[I9 NV] Failed to set employer business name', error);
    }
    ...
    try {
      ...
      if (widgets.length > 0) {
        ...
        console.warn('[I9 NV] Employer signature field has no widgets');
      }
    } catch (error) {
      console.warn('[I9 NV] Failed to embed employer signature image', error);
    }
    ...
    console.error('[I9 NV] PDF generation error:', error);
