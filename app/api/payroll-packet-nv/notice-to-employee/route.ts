  ...
  try {
    ...
    const removeFieldFromPdf = (fieldName: string) => {
      try {
        ...
        console.warn(`[NOTICE_TO_EMPLOYEE_NV] Field not found or removal failed: ${fieldName}`, error);
      }
    };
    ...
    try {
      ...
      if (widgets.length > 0) {
        ...
        console.warn('[NOTICE_TO_EMPLOYEE_NV] Employer signature field has no widgets');
      }
    } catch (error) {
      console.warn('[NOTICE_TO_EMPLOYEE_NV] Failed to embed employer signature image', error);
    }
    ...
    try {
      ...
      console.warn('[NOTICE_TO_EMPLOYEE_NV] Failed to pre-check Commission checkbox', error);
    }
    ...
    console.error('Notice to Employee PDF error (NV):', error);
