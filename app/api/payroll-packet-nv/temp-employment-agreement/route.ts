...
export async function GET(request: Request) {
  try {
    ...
    console.log('[NV] Cookie-based auth:', {
      hasUser: !!user,
      userId: user?.id
    });
    ...
    if (!user) {
      ...
      if (token) {
        console.log('[NV] Validating Bearer token...');
        ...
        user = tokenUser?.user;
        console.log('[NV] Bearer token validation:', {
          hasUser: !!user,
          userId: user?.id
        });
      }
    ...
    if (user) {
      ...
      console.log('[NV] Profile query result:', { profile, error });
    ...
    console.log('[NV] Final user name:', userName);
    ...
    console.log('NV - Page size:', lastPageSize);
    console.log('NV - Total pages:', pages.length);
    ...
    if (userName) {
      ...
      console.log('[NV] Drawing user name on first page at position (100, 570) size 9');
    }

    // BACK BUTTON
    const backButtonX = 50, backButtonY = 100, backButtonWidth = 100, backButtonHeight = 32;
    const backLinkAnnot = pdfDoc.context.obj({ Type: PDFName.of('Annot'), Subtype: PDFName.of('Link'), Rect: [backButtonX, backButtonY, backButtonX + backButtonWidth, backButtonY + backButtonHeight], Border: [0, 0, 0], C: [0.5, 0.5, 0.5], A: pdfDoc.context.obj({ S: PDFName.of('URI'), URI: PDFString.of('/payroll-packet-nv/form-viewer?form=notice-to-employee') }) });
    ...
    const continueLinkAnnot = pdfDoc.context.obj({ Type: PDFName.of('Annot'), Subtype: PDFName.of('Link'), Rect: [continueButtonX, continueButtonY, continueButtonX + continueButtonWidth, continueButtonY + continueButtonHeight], Border: [0, 0, 0], C: [0.1, 0.46, 0.82], A: pdfDoc.context.obj({ S: PDFName.of('URI'), URI: PDFString.of('/payroll-packet-nv/form-viewer?form=meal-waiver-6hour') }) });
    ...
    return new NextResponse(Buffer.from(pdfBytes), { status: 200, headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'inline; filename="NV_Temporary_Employment_Commission_Agreement.pdf"', 'Content-Security-Policy': "default-src 'self'", 'X-Content-Type-Options': 'nosniff' } });
  } catch (error: any) {
    console.error('NV Temp Employment Agreement PDF error:', error);
    return NextResponse.json({ error: 'Failed to generate NV Temp Employment Agreement PDF', details: error.message }, { status: 500 });
  }
}
